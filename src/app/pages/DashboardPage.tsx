...
export default function DashboardPage() {
  ...
  return (
    ...
    >
      <PageHero
        label="DASHBOARD"
        heading={`Welcome back, ${user?.name?.split(" ")[0] ?? "User"}.`}
        subtext="Your community hub — requests, matches, and trust signals in one place."
      />
      ...
      <div className="px-4 md:px-6 py-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          {[
            { label: "Trust Score", value: `${user?.trustScore ?? 0}%`, desc: "Your current community trust level" },
            { label: "Contributions", value: user?.contributions ?? 0, desc: "Total resolved requests and replies" },
            { label: "Notifications", value: unreadCount, desc: "Unread alerts and activity" },
          ].map((stat) => (
            <div
              key={stat.label}
            ...
            >
              ...
              <p className="text-xs text-gray-400" style={{ lineHeight: "1.5" }}>{stat.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-5">
          ...
          <button
            ...
            className="text-sm font-medium text-gray-700 px-5 py-2.5 rounded-full border hover:bg-gray-50 transition-colors"
